    ...
    if (isTemplateValid) {
        if (isCustomTemplate(rawTemplate)) {
            ...
            try {
                ...
                console.log(`%s There is no such template: ${chalk.white.bgBlack(templatePath)}. Check the specified path`, chalk.yellow.bold('WARNING'));
                return undefined;
            }
        }
        return template;
    }
    console.log(`%s You passed incorrect template: ${chalk.white.bgBlack(rawTemplate)}. List of supported templates: ${templates.join(', ')}`, chalk.yellow.bold('WARNING'));
    console.log(`%s For custom templates use this syntax: ${chalk.white.bgBlack('file:./path/to/custom/template')}`, chalk.yellow.bold('WARNING'));
    return undefined;
}
